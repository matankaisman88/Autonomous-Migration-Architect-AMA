...
def _schema_yml_for_model(model_name: str, mapped_columns: list[MappingRow]) -> str:
    ...
    for row in mapped_columns:
        alias = str(row.english_alias or "").strip()
        source = str(row.hebrew_name or "").strip()
        if not alias or alias in seen:
            continue
        seen.add(alias)
        lines.append(f"      - name: {alias}")
        lines.append(
            f"        description: {json.dumps(f'Source column `{source}`', ensure_ascii=False)}"
        )
    if not seen:
        lines.append("      - name: generated_column")
        lines.append("        description: Generated by AMA agentic extension")
    return "\n".join(lines) + "\n"
...
def _normalize_column(col: str | dict[str, Any]) -> tuple[str, str]:
    if isinstance(col, dict):
        return str(col.get("name") or "").strip(), str(col.get("type") or "").strip()
...
def generate_model_artifact(
    ...
    target_dialect: TargetDialect,
    source_ddl_columns: list[str | dict[str, Any]] | None = None,
    *,
    broken: bool,
    rationale: str,
    ...
    for c in (raw_columns or []):
        name, _ctype = _normalize_column(c if isinstance(c, dict) else str(c))
        if name:
            normalized_raw.append(name)
    normalized_ddl: list[str] = []
    for c in source_ddl_columns:
        name, _ctype = _normalize_column(c if isinstance(c, dict) else str(c))
        if name:
            normalized_ddl.append(name)
    normalized_raw = list(dict.fromkeys(normalized_raw))
    normalized_ddl = list(dict.fromkeys(normalized_ddl))

    raw_set = {str(c) for c in normalized_raw}
    extra_from_ddl = [str(c) for c in normalized_ddl if str(c) not in raw_set]
    ddl_only_cols = set(extra_from_ddl)
    all_columns = list(dict.fromkeys(list(normalized_raw) + extra_from_ddl))
    ...
    artifact = ModelArtifact(
        ...
        model_name=model_name,
        sql=sql,
        schema_yml=_schema_yml_for_model(model_name, mapped),
        mapping_rows=mapped,
