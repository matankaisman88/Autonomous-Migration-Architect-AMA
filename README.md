...
- **Migration Agent (chat-first AI cockpit)** — Goal-oriented dbt migration with a human approval gate (`request_write_permission`), table-focused intelligence feed ordering, per-wave progress tracking, automatic dbt test + fix loop, synthetic sample fallback when source DuckDB tables are unavailable, dynamic mapping headers (Hebrew vs source column), explicit manual-edit save button before approval, and role-labeled collaboration trace (**Architect / Developer / QA Lead**) in the Agent tab.
- **Execution resiliency (dashboard + dbt)** — Automatic recovery from common local execution blockers: schema YAML sanitization for legacy description payloads, generated SQL trailing-semicolon cleanup, retry/fallback when requested dbt target is not configured, transient DuckDB file-lock retries, and best-effort auto-bootstrap of missing local DuckDB source schema/table relations used by generated models.
- **Self-healing SQL generation** — `propose_dbt_model` now includes pre-approval SQL QA using `sqlglot`, bounded Developer self-correction (max 3 attempts), and explicit `CRITICAL_REASON` escalation when automated correction is exhausted.
- **Migration data-preservation guardrail (important)** — Migration defaults to full-row copy semantics. The generator blocks LLM SQL drafts that introduce business row filters (for example `WHERE status='unpaid'`) unless explicitly requested, and falls back to deterministic all-rows SQL to prevent silent data loss.
...
| `ama.dbt_migration`  | dbt migration artifacts, role-specialized generation, sqlglot syntax gate, bounded self-healing loop                       |
...
| **Tables**             | Per-table merge breakdown + optional **pyvis** lineage neighborhood graph; manifest-unknown tables (vs `ddl_manifest_table_keys`) render as **warning** styling; successful table migrations are marked `✅ Done` and can be hidden from active actions |
| **Bulk Migration**     | Queue-aware mass execution for GREEN tables with background worker status, contract preview, live progress polling, completion notices, and post-run `migrated` status reconciliation (remaining actionable GREEN list only) |
...
| **Migration Agent**        | Chat-first migration workflow with tool orchestration (`list_waves`, `analyze_schema`, `propose_dbt_model`, `execute_dbt_test`, `apply_fix`) and mandatory write approval gate anchored at the bottom; table-focused feed (hides unrelated/duplicate tool noise), explicit `Save Edited SQL` action, per-wave progress tracking, and a role-labeled collaboration trace in `st.status` (Architect/Developer/QA Lead). |

> Important migration rule: unless you explicitly ask for a filtered/business dataset, generated migration SQL preserves all source rows. Business `WHERE` predicates are rejected as unsafe defaults for full-data migration.
