...
- **Migration Agent (chat-first AI cockpit)** — Goal-oriented dbt migration with a human approval gate (`request_write_permission`), table-focused intelligence feed ordering, per-wave progress tracking, automatic dbt test + fix loop, synthetic sample fallback when source DuckDB tables are unavailable, dynamic mapping headers (Hebrew vs source column), and explicit manual-edit save button before approval.
