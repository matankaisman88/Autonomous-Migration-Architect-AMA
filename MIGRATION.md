...
- **Missing source auto-bootstrap**: for local DuckDB execution, missing `schema.table` source relations referenced by generated SQL are created best-effort so `dbt run`/`dbt test` can proceed. Column lists are merged from report DDL, `live_data/<connection>/ddl/*.json`, `schema.yml`, and model SQL; existing stub columns are never dropped on re-bootstrap. Cockpit batch execution pre-bootstraps all Checkpoint-A models before the first wave; **Bulk migration** passes the same report/DDL context into batched `test_models_batch` validation (and per-table fix retries) so mapped projections are not incorrectly replaced by `SELECT *` passthrough. Generated models are written to and executed from `dbt_project/models/ama_generated/`. This validates SQL locally only — it does not load production data.
